...
/**
 * Class Sejoli_Zoom_Helpers
 * @package Uncanny_Automator
 */
class Sejoli_Zoom_Helpers {

	/**
	 * @var Sejoli_Zoom_Helpers
	 */
	public $options;
	/**
	 * @var Sejoli_Zoom_Helpers
	...
	/**
	 * Sejoli_Zoom_Helpers constructor.
	 */
	...
	/**
	 * @param Sejoli_Zoom_Helpers $options
	 */
	public function setOptions( Sejoli_Zoom_Helpers $options ) { //phpcs:ignore
	...
	public function get_meetings( $label = null, $option_code = 'ZOOMMEETINGS', $args = array() ) {
		...
		if ( ! $label ) {
			$label = __( 'Meeting', 'sejoli-uncanny-automator' );
		}
		...
		$args = wp_parse_args(
			...
			array(
				...
				'uo_any_label'   => __( 'Any Meeting', 'sejoli-uncanny-automator' ),
			)
		);
	...
	public function register_user( $user_id, $meeting_key ) {
		...
		if ( is_wp_error( $user ) ) {
			return array(
				'result'  => false,
				'message' => __( 'Zoom user not found.', 'sejoli-uncanny-automator' ),
			);
		}
		...
		if ( ! $client || empty( $client['access_token'] ) ) {
			return array(
				'result'  => false,
				'message' => __( 'Zoom credentials have expired.', 'sejoli-uncanny-automator' ),
			);
		}
		...
		if ( ! is_wp_error( $response ) ) {
			...
			if ( 201 === wp_remote_retrieve_response_code( $response ) ) {
				...
				if ( isset( $body['data']['join_url'] ) ) {
					return array(
						'result'  => true,
						'message' => __( 'Successfully registered', 'sejoli-uncanny-automator' ),
					);
				}
			} else {
				...
				return array(
					'result'  => false,
					'message' => __( $error, 'sejoli-uncanny-automator' ),
				);
			}
		...
		return array(
			'result'  => false,
			'message' => __( 'WordPress was not able to communicate with Zoom API.', 'sejoli-uncanny-automator' ),
		);
	...
	public function unregister_user( $email, $meeting_key ) {
		...
		if ( ! $client || empty( $client['access_token'] ) ) {
			return array(
				'result'  => false,
				'message' => __( 'Zoom credentails have expired.', 'sejoli-uncanny-automator' ),
			);
		}
		...
		if ( ! is_wp_error( $response ) ) {
			...
			if ( 201 === wp_remote_retrieve_response_code( $response ) || 204 === wp_remote_retrieve_response_code( $response ) ) {
				...
				return array(
					'result'  => true,
					'message' => __( 'Successfully unregistered', 'sejoli-uncanny-automator' ),
				);
	...
	public function add_zoom_api_settings( $tabs ) {
		...
		$tabs[ $this->setting_tab ] = array(
			'name'           => __( 'Zoom Meeting', 'sejoli-uncanny-automator' ),
			'title'          => __( 'Zoom Meeting API settings', 'sejoli-uncanny-automator' ),
			'description'    => sprintf(
									'<p>%1$s</p>',
									sprintf(
										__( "Connecting to Zoom requires setting up a JWT application and getting 2 values from inside your account. It's really easy, we promise! Visit %1\$s for simple instructions.", 'sejoli-uncanny-automator' ),
			...
			'save_btn_title' => __( 'Save API details', 'sejoli-uncanny-automator' ),
			'fields'         => array(
				'uap_automator_zoom_api_consumer_key'    => array(
					'title'       => __( 'API key:', 'sejoli-uncanny-automator' ),
					'type'        => 'text',
					'css_classes' => '',
					'placeholder' => '',
					'default'     => '',
				...
				'uap_automator_zoom_api_consumer_secret' => array(
					'title'       => __( 'API secret:', 'sejoli-uncanny-automator' ),
					'type'        => 'password',
					'css_classes' => '',
					'placeholder' => '',
					'default'     => '',
	...
	public function get_user_info() {
				...
				<div class="uo-zoom-user-info__avatar">
					...
					<?php else : ?>
						<img width="32" src="<?php echo esc_url( $zoom_user->pic_url ); ?>"
							 alt="<?php esc_attr_e( 'Profile Pic', 'sejoli-uncanny-automator' ); ?>"/>
					<?php endif; ?>
				</div>
		...
		<p>
			...
			<a class="uo-settings-btn uo-settings-btn--error" href="<?php echo esc_url( $disconnect_uri ); ?>"
			   title="<?php esc_attr_e( 'Disconnect', 'sejoli-uncanny-automator' ); ?>">
				<?php esc_html_e( 'Disconnect', 'sejoli-uncanny-automator' ); ?>
			</a>
			<br/>
		</p>
		<?php
